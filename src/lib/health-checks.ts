...
export async function testSimulatorHandshake(): Promise<FetchResult> {
  if (!hasEffectiveApiKey()) {
    return {
      ...
      durationMs: 0,
      message:
        "API key is not configured. Provide one above or set NEXT_PUBLIC_BACKEND_TOKEN (or another supported env) to run the handshake check."
    };
  }
  ...
  if (!result.ok && result.status === 401) {
    return {
      ...result,
      message:
        "Handshake request failed: backend rejected the API key. Double-check the value provided above or the NEXT_PUBLIC_BACKEND_TOKEN environment variable."
    };
  }
